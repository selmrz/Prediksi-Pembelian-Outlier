...
produk_list = df['ProductName'].dropna().unique().tolist()  # dropna() untuk menghindari nilai NaN
...
negara_list =  sorted (df['Country'].dropna().unique().tolist())  # dropna() untuk menghindari nilai NaN
...
try: 
    model = joblib.load('logistic_regression_outlier.pkl') 
except FileNotFoundError: 
    st.error("Model file not found. Please make sure 'logistic_regression_outlier.pkl' is in the same directory.") 
    st.stop() 
